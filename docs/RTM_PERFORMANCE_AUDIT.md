...
Safe future offline production would need a deterministic record of recipe version, input/output and adjacent inventory ownership, upgrade and energy availability over elapsed time, transfer order, and commit/revalidation rules. None is available from a due tick alone, so this phase keeps unloaded controllers dormant. Chemplant is not ready for this exact cutover: it has multiple lanes and shared fluid tanks, direct slot/tank writes, external fluid network changes, and independent completion order to audit before scheduling them.
