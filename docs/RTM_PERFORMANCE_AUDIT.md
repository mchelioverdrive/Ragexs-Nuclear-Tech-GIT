...
This document records the current machine execution architecture and source-confirmed performance work. `MachineRuntime` is now the primary execution model for migrated production families; the remaining legacy systems and their current constraints are listed below. It complements `RTM_Performance_Architecture_Audit.docx`, which remains the original audit snapshot.

## Current machine-runtime status — 2026-09-26

### Scope and ownership

The machine-source inventory covers ten behavior categories: single-operation production, parallel production, fluid processing, generation, energy storage/conversion, thermal/pressure simulation, multiblock controllers, environmental machinery, transfer/logistics, and simulation-dependent machinery. A source inventory of `src/main/java/com/hbm/tileentity/machine/` found 193 concrete TileEntities descending from the shared machine, loaded-tile, polluting, foundry, reactor, storage, or related support bases. Eighty-seven concrete classes currently opt into `MachineRuntime`, directly or through a runtime-enabled base. This is a source count, not a claim that all 193 are production controllers; the legacy census includes storage, ports, reactor members, and utility blocks.

The 87 runtime-covered classes comprise 76 strategy declarations (seven are abstract shared bases) and eighteen concrete descendants that inherit a strategy: the eight-lane Assembly Factory and Chemfac, the electric and steam pump variants, the Deuterium Tower and Atmo Tower, Foundry Basin, Foundry Mold, Heater Firebox, Heater Oven, Chimney Brick, Chimney Industrial, Radiator, powered condenser, both Heat Boilers, and the large and small condenser towers. All migrated families are server-thread owned and opt into event-driven reevaluation. They use typed scheduled work where progress or continuous accounting is required, and COARSE_5/20/100 only where direct mutation, registry state, environment, topology, or synchronization still lacks a reliable event. None of these declarations uses `REALTIME` as a substitute for changing the production path; actively simulated work goes through runtime callbacks.

### Runtime migration matrix

| Family | Runtime-covered controllers | Execution model and retained cadence |
| --- | --- | --- |
| Furnaces | `TileEntityMachineElectricFurnace`, `TileEntityFurnaceBrick`, `TileEntityFurnaceIron`, `TileEntityFurnaceSteel`, `TileEntityMachineArcFurnace`, `TileEntityMachineArcFurnaceLarge` | Dirty recipe/upgrade eligibility; scheduled active work. The large arc furnace keeps one ordered aggregate batch over twenty material slots and a shared liquid buffer, with one scheduled lid/progress/pour task. Steel retains its ordered heat/fuel lanes and external heat-source accounting. |
| Firebox thermal controllers | `TileEntityHeaterFirebox`, `TileEntityHeaterOven` | The shared base schedules active fuel burn, heat loss, smoke transfer, and the oven's adjacent heat-source draw. Waterlogging is refreshed every five ticks; direct fuel-slot changes are fingerprinted and external heat availability is checked every five ticks. |
| Moving-item press | `TileEntityConveyorPress` | Press motion and entity interception run in a scheduled task while powered and equipped; power-neighbor discovery refreshes every twenty ticks. The default 100-quantum-per-tick draw is 1,000 W. Press position, retraction phase, and delay reconstruct from NBT. |
| Foundry casting | `TileEntityFoundryBasin`, `TileEntityFoundryMold`, `TileEntityMachineStrandCaster` | The shared foundry-casting controller schedules full-mold cooling and output. Crucible material input and inventory changes invalidate eligibility; Strand Caster also observes water/steam tank writes, polls direct item/material changes every five ticks, and renews pipe connections every twenty ticks. Its batch threshold and 200-tick inactivity flush remain authoritative. |
| Assembly and chemical factories | `TileEntityMachineAssembler`, `TileEntityMachineAssemfac`, `TileEntityMachineChemplant`, `TileEntityMachineChemfac` | Shared lane bases use task slots. Assembly Factory and Chemfac each expose eight lanes; callbacks execute deterministically in lane order and recheck shared resources before mutation. |
| General production and assembly control | `TileEntityMachineAutocrafter`, `TileEntityMachineCompressor`, `TileEntityMachineCentrifuge`, `TileEntityMachineCrystallizer`, `TileEntityMachineCyclotron`, `TileEntityMachineDischarger`, `TileEntityMachineDriveProcessor`, `TileEntityMachineEPress`, `TileEntityMachineExposureChamber`, `TileEntityMachineFunnel`, `TileEntityMachineMixer`, `TileEntityMachinePress`, `TileEntityMachineRocketAssembly`, `TileEntityMachineSchrabidiumTransmutator`, `TileEntityMachineShredder`, `TileEntityMachineSolderingStation`, `TileEntityMachineVacuumCircuit`, `TileEntityMicrowave`, `TileEntityMachineArcWelder` | Dirty operation eligibility and typed transitions; active per-tick resource accounting remains where it defines the existing rate. Drive Processor neighbor discovery moved from every tick to COARSE_20. Rocket Assembly VAB structure/platform state rebuilds on inventory or lifecycle invalidation, with bounded inventory and drive-target checks. |
| Oil and fluid chemistry | `TileEntityMachineAlkylation`, `TileEntityMachineCatalyticCracker`, `TileEntityMachineCatalyticReformer`, `TileEntityMachineCoker`, `TileEntityMachineHydrotreater`, `TileEntityMachineLiquefactor`, `TileEntityMachineRefinery`, `TileEntityMachineShredderLarge`, `TileEntityMachineSolidifier`, `TileEntityMachineVacuumDistill`, `TileEntityMachineCryoDistill`, `TileEntityMachineMilkReformer`, `TileEntityElectrolyser`, `TileEntityMachineGasCent` | Cached eligibility, fluid event invalidation, scheduled work and output transfer. Direct inventory/container writes are covered by the individual machine's bounded fingerprint where the base hook cannot see them. |
| Environmental collection, reflection, and fluid movement | `TileEntityAirPump`, `TileEntityAirScrubber`, `TileEntityAlgaeFilm`, `TileEntityAtmoExtractor`, `TileEntityDeuteriumExtractor`, `TileEntityMachinePumpElectric`, `TileEntityMachinePumpSteam`, `TileEntityMachineGasDock`, `TileEntityAshpit`, `TileEntitySolarMirror`, plus `TileEntityAtmoTower` and `TileEntityDeuteriumTower` | Scheduled environmental/pump work; cached environment and connection data; coarse topology or inventory checks as required. Solar Mirror checks for startup every twenty ticks while idle, then refreshes light and delivers heat each active tick. The water-pump variants share one runtime-enabled base. |
| Power-demand control | `TileEntityMachineDetector` | Energy changes invalidate the active state; active demand consumes one half-joule quantum each tick (10 W at 20 TPS) in a scheduled transition. Power-neighbor discovery renews every twenty ticks. Energy is stored in existing Joule-quantum NBT form. |
| Continuous generation and heat-to-power | `TileEntityMachineAmgen`, `TileEntityMachineDiesel`, `TileEntityMachineMiniRTG`, `TileEntityMachineRTG`, `TileEntityMachineSolarPanel`, `TileEntityMachineSPP`, `TileEntityStirling` | Diesel fuel conversion, battery charging, pollution, power and smoke transfer run as an active task. Tank writes are observed, waterlogging and direct inventory mutations have a five-tick fallback, and connection subscriptions refresh every twenty ticks. Mini RTG, RTG pellet decay, and Stirling retain active per-tick generation/heat/export accounting as typed runtime tasks. RTG uses slot mutation hooks and a five-tick compact fingerprint; its electricity packet keeps the twenty-tick baseline. Solar panel checks light/time every tick while active and uses a twenty-tick poll to restart at a daylight boundary. SPP moves its segmented tower validation to twenty-tick polling and schedules generation/export only while output or stored energy exists. Amgen's neighboring geothermal/lava source discovery runs every twenty ticks while idle; active lava conversion chances and power export retain their per-tick cadence. Generator rates remain Joule quanta per tick internally and expose watts through the shared conversion. Stirling retains per-tick heat-source exchange, overspeed failure, and energy export. |
| Smoke handling and ash capture | `TileEntityChimneyBrick`, `TileEntityChimneyIndustrial` | Smoke intake marks the logical chimney dirty; pending ash/soot transfer runs once as a scheduled task and is persisted until delivered to a loaded Ashpit. The active particle countdown remains a one-tick visual state, while smoke pipe subscriptions and packet baseline refresh every twenty ticks. Client particle generation stays client-owned. |
| Fluid release | `TileEntityMachineDrain` | Owner-observed tank changes start or stop scheduled per-tick spill behavior; explosive and volatile-fluid world effects retain their active cadence. Neighbor connections and idle tank sync renew every twenty ticks. |
| Fuelled physical processing | `TileEntityMachineAutosaw` | Exact twenty-tick fuel consumption and neighbor renewal use a typed transition; active entity, tree, and block interaction stays on its one-tick scheduled cadence. Fluid changes are owner-observed and idle saws have no per-tick server task. |
| Radiation generation | `TileEntityMachineRadGen` | Twelve lanes preserve transfer-before-load and ascending-lane order. Stored energy remains quanta; the machine also exposes its generation rate in watts. |
| Steam condensation and recovery | `TileEntityCondenser`, `TileEntityRadiator`, `TileEntityCondenserPowered`, `TileEntityTowerLarge`, `TileEntityTowerSmall` | The common controller wakes on tank or power changes and runs bulk conversion, evaporation checks, and output transfer only while input is eligible, output remains, or the 20-tick active display timer is running. Owned tanks observe HBM/Forge fluid writes. Pipe subscriptions refresh every twenty ticks. Powered condenser energy is charged in Joule quanta per converted millibucket, and the corresponding rate is exposed in watts. The large and small tower variants inherit the same controller and retain their client-only steam effects. |
| Electric heat source | `TileEntityHeaterElectric` | Energy and setting changes start scheduled heat work; active heat transfer and decay retain the existing per-tick cadence. Power subscriptions refresh every twenty ticks, and an idle heater checks its adjacent heat source on that boundary. Its configured input draw is exposed in watts. Client audio remains client-owned. |
| Heat boilers | `TileEntityHeatBoiler`, `TileEntityHeatBoilerIndustrial` | A shared runtime base owns tank observation, thermal exchange, bulk conversion, pipe subscription, output flow, and lifecycle binding. Heat and output retain one-tick simulation while active; empty boilers wake every twenty ticks for adjacent heat sources and firestorm conditions. The standard boiler retains its blocked-output explosion; the industrial boiler retains its larger port geometry and client audio. |
| Heat exchanger | `TileEntityHeaterHeatex` | Tank and configuration changes reevaluate conversion; heat decay, conversion, and output transfer run as a scheduled active task. Cached port geometry and twenty-tick fluid subscriptions replace per-tick port allocation and discovery. External heat extraction remains server-thread owned and marks persisted heat state dirty. |
| Steam engine | `TileEntitySteamEngine` | Steam-to-spent-steam conversion, rotor acceleration, power export, and output transfer run in one scheduled task while active; tank writes wake eligibility, ports are cached, and subscriptions plus idle packet correction run every twenty ticks. Client rotor interpolation remains client-side. Generation is exposed as watts from current conversion capacity. |
| Gas flare and wood burner | `TileEntityMachineGasFlare`, `TileEntityMachineWoodBurner` | Gas venting/combustion and solid/liquid fuel accounting run only while eligible or while energy remains for export. Inventory, configuration, fluid, and energy changes wake the task; fixed ports are cached and subscriptions refresh every twenty ticks. Active air consumption, entity damage, pollution, smoke, and energy export retain their original server cadence. Both expose generated power as watts. |
| Oil-fired heat source | `TileEntityHeaterOilburner` | Tank, control, and waterlogging changes affect eligibility; active burning, heat decay, air consumption, pollution, and smoke transfer use scheduled work. Waterlogging is sampled every five ticks, pipe connections every twenty, and the fixed port positions are cached. |

### Shared invalidation and caches

- `TileEntityMachineBase`'s normal inventory setters, split/extract paths, and container slot paths signal inventory/recipe dirtiness and the existing PowerNet invalidation. Migrated machines that write `slots[]` directly either signal the common hook at the mutation or use a compact COARSE_5 fingerprint; the fallback marks dirty and does not run a recipe scan.
- `TileEntityFoundryCastingBase` now signals mold/output inventory changes, and the loaded foundry base signals material changes from crucible flows. Strand Caster's water and steam tanks use the shared owner callback, so pipe, container, and direct tank writes invalidate its batch eligibility.
- `FluidTank` has an optional owner listener. Only tanks owned by opted-in machines are attached through `trackMachineFluidTank()`. `setFill`, `setTankType`, `changeTankSize`, and pressure changes notify the owner, covering HBM pipe/container/API writes and direct tank setters without changing unowned global tanks. Runtime-owned multi-write operations use a small suppression scope and reevaluate once after the operation. NBT restoration and packet deserialization remain direct restoration paths; they happen before server rebind or on the client and are followed by normal lifecycle reconstruction.
- `SerializableRecipe.getRegistryRevision()` advances after a complete recipe registration pass. Runtime recipe caches that use this registry compare its revision in their bounded registry poll and refresh only when it changes. Other recipe families retain their existing indexes or machine-specific caches; there is no new cache per tile.
- Upgrade state uses the shared upgrade dirty cause and existing `UpgradeManagerNT` APIs. Migrated machines refresh upgrade-derived timing/power on relevant dirty causes and reserve bounded checks for direct slot mutation.
- Neighbor connection discovery is moved to coarse buckets or cached geometry. Fluid send/receive and required power accounting keep their old active-work cadence for now.

### Joules, watts, and accounting

Machine buffers and transfers use exact half-joule `energyQuanta` (one quantum = 0.5 J). A rate expressed as quanta per simulation tick is converted with `EnergyUnits.quantaPerTickToWatts`; per-tick accounting converts the configured watt draw back with `wattsToQuantaPerTick`, preserving the already-migrated balance. Watts describe rate, quanta describe stored or consumed energy. The 20 TPS simulation rate remains the accounting basis for callbacks that intentionally retain one-tick work. `TileEntityMachineRadGen` reports output power in watts; this migration does not claim that every legacy generator or every energy network has moved to runtime ownership.

### Persistence, lifecycle, and multiblock behavior

Machine generation identity and schedule ownership remain in `MachineRuntime`; task queues, bindings, and recipe objects are transient. Migrated controllers save authoritative progress, power, heat, tanks, and machine state in their existing NBT plus only the operation metadata needed to reconstruct eligibility and the next boundary. A bind after chunk load reevaluates state and recreates schedules; unload unbinds without offline catch-up. The eight-lane assembler and chemical-factory controllers are migrated as logical controllers; arbitrary RBMK, foundry flow endpoints, reactor, and other member TileEntities remain on their subsystem-owned paths. Foundry casting controllers now inherit the loaded-tile lifecycle needed for safe bind/unbind and reconstruct their mold/cooloff work from existing material, inventory, and tank state. The Rocket Assembly VAB controller now rebuilds its rocket and platform/spire state on inventory/lifecycle invalidation, with a COARSE_5 inventory audit and COARSE_20 station-target/sync refresh; it no longer rebuilds `RocketStruct` every server tick.

### One-tick work and bounded polls still present

One-tick scheduled callbacks remain for active progress/energy accounting, generators that transfer each tick, environmental rates, coupled rotor/heat simulation, and fluid output that drains each tick. The current immediate-reschedule call sites are in `TileEntityFoundryCastingBase`, `TileEntityMachineStrandCaster`, `TileEntityMachineAssemblerBase`, `TileEntityMachineChemplantBase`, `TileEntityMachineArcFurnace`, `TileEntityMachineArcFurnaceLarge`, `TileEntityMachineArcWelder`, `TileEntityMachineAutocrafter`, `TileEntityMachineCompressor`, `TileEntityMachineCryoDistill`, `TileEntityMachineCyclotron`, `TileEntityMachineDischarger`, `TileEntityMachineDriveProcessor`, `TileEntityMachineEPress`, `TileEntityMachineExposureChamber`, `TileEntityMachineFunnel`, `TileEntityMachineGasCent`, `TileEntityMachineGasDock`, `TileEntityMachineMilkReformer`, `TileEntityMachinePress`, `TileEntityMachineRadGen`, `TileEntityMachineSchrabidiumTransmutator`, `TileEntityMachineShredder`, `TileEntityMachineSolderingStation`, `TileEntityMachineVacuumCircuit`, `TileEntitySteamEngine`, `TileEntityHeaterHeatex`, `TileEntityHeaterOilburner`, `TileEntityMachineGasFlare`, `TileEntityMachineWoodBurner`, the brick/iron/steel furnaces, the electrolyser, the environmental collectors, pumps, and the listed oil processors. Some callbacks schedule at 5, 10, or 20 ticks when that is their actual boundary. This is conservative migration accounting, not the later global cadence optimization.

COARSE_5 is used for direct slot/container fingerprints and close-range transfer work. COARSE_20 is used for connection renewal, environment refresh, station-target refresh, periodic synchronization, or external state that has no hook. COARSE_100 is limited to registry-size/revision or similarly slow fallback checks. The world runtime spreads keys across shared buckets; callbacks compare compact state and mark dirty instead of scanning recipes. No migrated machine polls from a worker thread or simulates while unloaded.

The structural work removed from migrated inactive machines includes full per-tick eligibility and recipe/upgrade checks, repeated connection discovery, repeated input/tank scans, and repeated environmental lookup where explicitly cached. The large arc furnace caches its twenty recipe results between inventory, mode, and registry changes; Rocket Assembly no longer reconstructs its rocket every server tick; the steam engine, heat exchanger, gas flare, and wood burner no longer run their server loops while idle and cache fixed port geometry. Active one-tick work remains where it still represents the original power, fuel, heat, environmental, or fluid-transfer cadence. These changes establish complexity reductions from source structure; no profiler or in-game timing result was collected in this task.

### Legacy machine-related classes and current reason

The following concrete descendants still have legacy ticking. Names are grouped by the common authoritative subsystem they currently use; the reason applies to every class in its row. This is not a claim that all are production machines, nor that every row is the final architecture.

| Legacy subsystem and concrete constraint | Remaining classes |
| --- | --- |
| Persistent containers and passive storage/API endpoints: fluid/item contents are changed by vanilla, HBM, Forge, addon, or neighboring-machine APIs; they have no single scheduled operation owner. Battery subclasses are also live PowerNet endpoints whose network manager owns endpoint membership and transfer cadence. Doors retain their block-state/redstone behavior rather than production work. | `TileEntityBarrel`, `TileEntityBlastDoor`, `TileEntityMachineBAT9000`, `TileEntityMachineOrbus`, `TileEntityCrateDesh`, `TileEntityCrateIron`, `TileEntityCrateSteel`, `TileEntityCrateTemplate`, `TileEntityCrateTungsten`, `TileEntityFileCabinet`, `TileEntityMassStorage`, `TileEntitySafe`, `TileEntityVaultDoor`, `TileEntityStorageDrum`, `TileEntityMachineBattery`, `TileEntityMachineFENSU`, `TileEntityMachineFluidTank` |
| Solar Boiler: output depends on sky visibility, day state, the current celestial sun-power multiplier, incoming water, and externally supplied heat; the environment/heat-source owner has no shared invalidation hook yet. | `TileEntitySolarBoiler` |
| Dual and combination furnaces: operation couples burn lanes, heat evolution, material state, and output readiness, so inventory invalidation alone cannot safely reconstruct their continuous state. | `TileEntityDiFurnace`, `TileEntityDiFurnaceRTG`, `TileEntityFurnaceCombination` |
| Combustion engine: each update coordinates sub-millibucket fuel remainder, piston grade, air/waterlogging, pollution, energy, and smoke; those dependencies do not yet share a reliable event owner. | `TileEntityMachineCombustionEngine` |
| Fraction tower: staged distillation depends on several fluid outputs and remote pipe acceptance while advancing fraction-specific state; the multi-tank stage boundaries need a controller-owned reconstruction path. | `TileEntityMachineFractionTower` |
| Turbines: generation couples steam pressure, exhaust room, energy-buffer room, rotor/start-stop state, and network export; safe scheduling needs pressure/output invalidation across the connected turbine path. | `TileEntityMachineTurbine`, `TileEntityMachineLargeTurbine`, `TileEntityMachineTurbineGas`, `TileEntityMachineTurbofan` |
| Pyro and rotary furnaces: their material/temperature cycles interact with physical heat and fuel state, and their completion boundary is not represented by a single persistent operation owner. | `TileEntityMachinePyroOven`, `TileEntityMachineRotaryFurnace` |
| Plasma Heater: compatible fuel pairing, staged startup energy, short-lived plasma containment, and injection into ITER/HTRF controllers form one cross-controller operation without a current shared lifecycle binding. | `TileEntityMachinePlasmaHeater` |
| Disabled generator implementation: its only production loop and client rotation code are commented out, so there is no active machine work to migrate; runtime adoption would reactivate intentionally disabled generation. | `TileEntityMachineIGenerator` |
| Reactor, breeding, isotope, and accelerator simulation: state is shared across controller/member tiles or changes with neutron flux, radiation, fuel, temperature, pressure, particle state, or multiblock structure. Members do not have a single controller binding or reliable structure-change invalidation yet. | `TileEntityCore`, `TileEntityCoreEmitter`, `TileEntityCoreInjector`, `TileEntityCoreReceiver`, `TileEntityCoreStabilizer`, `TileEntityFEL`, `TileEntityHadron`, `TileEntityHadronDiode`, `TileEntityHadronPower`, `TileEntityICF`, `TileEntityICFController`, `TileEntityICFPress`, `TileEntityITER`, `TileEntityMachineRadiolysis`, `TileEntityMachineReactorBreeding`, `TileEntityMoltenSaltReactor`, `TileEntityMoltenSaltReactorPort`, `TileEntityPWRController`, `TileEntityReactorControl`, `TileEntityReactorResearch`, `TileEntityReactorZirnox`, `TileEntitySILEX`, `TileEntityWatz`, `TileEntityPileBreedingFuel`, `TileEntityPileFuel`, `TileEntityPileSource`, `TileEntityRBMKAbsorber`, `TileEntityRBMKBlank`, `TileEntityRBMKBoiler`, `TileEntityRBMKBurner`, `TileEntityRBMKControlAuto`, `TileEntityRBMKControlManual`, `TileEntityRBMKCooler`, `TileEntityRBMKConsole`, `TileEntityRBMKHeater`, `TileEntityRBMKInlet`, `TileEntityRBMKModerator`, `TileEntityRBMKOutgasser`, `TileEntityRBMKOutlet`, `TileEntityRBMKReflector`, `TileEntityRBMKRod`, `TileEntityRBMKRodReaSim`, `TileEntityRBMKStorage` |
| Physical material handling, mining, or world mutation: work discovers or changes blocks, entities, molten-material flow, drill surfaces, or large generated structures. Those mutations currently originate outside a single inventory/fluid transition and have no safe shared invalidation boundary. | `TileEntityCrucible`, `TileEntityFoundryChannel`, `TileEntityFoundryOutlet`, `TileEntityFoundrySlagtap`, `TileEntityFoundryTank`, `TileEntityMachineExcavator`, `TileEntityMachineMiningLaser`, `TileEntityMachineOilWell`, `TileEntityMachinePumpjack`, `TileEntityMachineFrackingTower`, `TileEntityMachineOreSlopper`, `TileEntitySawmill` |
| External transfer, scanning, control, or mission interaction: these poll or act on entities, world regions, remote controllers, propulsion state, or third-party APIs each tick. Their event source is outside the TileEntity inventory and needs an explicit endpoint/lifecycle bridge. | `TileEntityCharger`, `TileEntityChungus`, `TileEntityForceField`, `TileEntityMachineHephaestus`, `TileEntityMachineRadarLarge`, `TileEntityMachineRadarNT`, `TileEntityMachineStardar`, `TileEntityMachineTeleporter`, `TileEntityRadiobox`, `TileEntityRefueler`, `TileEntityTransporterRocket`, `TileEntityOrbitalStation`, `TileEntityOrbitalStationComputer`, `TileEntitySoyuzLauncher`, `TileEntityMachineHTR3`, `TileEntityMachineHTRF4`, `TileEntityMachineLPW2`, `TileEntityXenonThruster`, `TileEntityTesla` |
| Configurable compatibility machine: recipe shape, progress, energy, and pollution behavior are selected from external machine configuration rather than one of the stable indexed recipe registries, so a registry/config revision hook must be connected before caching eligibility. | `TileEntityCustomMachine` |

This census contains 106 legacy concrete descendants after the 87 runtime-covered classes are excluded from the 193-class machine-related inventory. Some listed support endpoints do not represent production work and should remain outside scheduling unless an event-driven task is introduced for a specific behavior. The listed production systems still need family-by-family migration; their reasons are the specific subsystem dependencies in each row, not diff size or age. Mining/physical machinery, heat/generation systems, and the coupled reactor families are the largest remaining production areas.

### Known correctness risks and follow-up

The migration has not been run in Minecraft. Highest regression risk is active timing/energy balance, direct `slots[]` or tank replacement paths missed by fingerprints, fluid routing and container interactions, deterministic same-tick lane completion, chunk unload/rebind, active/inactive block swaps, radiation/environment sampling, firebox waterlogging and oven heat-source refresh boundaries, oil-burner air/waterlogging boundaries, gas-flare entity damage/pollution, steam-engine rotor synchronization, and world-mutating multiblock rebuilds. The next pass should first finish remaining eligible production families, then replace one-tick accounting with resource-boundary/elapsed-time scheduling and replace bounded polls with shared mutation notifications. In-game save/reload, automation, recipe reload, multi-lane contention, power interfaces, fluid networks, and dedicated-server behavior remain required validation.

The final offline `compileJava` completed successfully with the installed Corretto Java 8 JDK. Earlier attempts first hit Gradle-cache access and a JRE-only `JAVA_HOME`; after selecting the installed JDK, the build compiled the changed sources. `git diff --check` also passed. Minecraft was not launched, so gameplay and lifecycle behavior remain unverified in runtime.
...
- Replaced per-call `SlotSignature[]` and `SlotSignature` creataon with reusable primitive/reference signature storage.
...
At the completion of Phase 0, `LEGACY` remained the default execution strategy, legacy `updateEntity()` methods still ran, and no existing machine had yet been opted into runtime-driven simulation. The Phase 1 section below records the first opt-in.
...
Machine simulation dirtiness remains independent of `markNetworkDirty()`. PowerNet remains an independent authority and scheduler; the electric furnace's `setPower()` exposed the future machine-energy invalidation seam while retaining its existing PowerNet invalidation. At Phase 0 completion the furnace was still `LEGACY`, so this initially had no runtime scheduling effect.
...
No machine gameplay loop, rendering state, PowerNet ownership, fluid-network ownership, external API, or legacy fallback was migrated or removed in Phase 0.
...
## 2026-09-25 20:21 — Phase 1 Electric Furnace migration

### Execution and energy accounting

`TileEntityMachineElectricFurnace` is the first production controller opted into `EVENT_DRIVEN | SCHEDULED | COARSE_20`. Its legacy `updateEntity()` no longer scans upgrades, looks up a smelting result, checks output capacity, advances progress, charges a battery, consumes energy, emits soot, or changes the lit block every TileEntity tick. That method now retains only the established 40-tick PowerNet connection subscription and the allocation-free dirty/baseline packet gate.

Inventory, recipe, configuration, lifecycle, and external energy changes drive reevaluation through the world runtime. Eligible active work owns one typed accounting transition. The transition remains one tick apart because the original furnace consumed `consumption` HE, discharged its battery slot, decremented cooldown, and conditionally emitted soot on actual server ticks; converting that behavior into a one-time reservation or a wall-clock completion would change starvation, charging, pollution, and upgrade semantics. The scheduled callback therefore performs only cheap accounting in the steady active path: cooldown, battery discharge, the power threshold, one progress increment, energy consumption, and the existing once-per-20-world-ticks soot event. Recipe/output validation is cached and is rerun on dirty input/output events and at the authoritative completion boundary. Upgrade formulas are recomputed only on relevant dirty work or the bounded compatibility poll.

An idle furnace with no battery work owns no transition and performs no recipe or upgrade work each tick. A powered eligible furnace, a draining battery, or a live cooldown owns the next accounting tick. PowerNet writes continue through `setPower()`, cancel the old accounting transition, and issue an `ENERGY` dirty cause; runtime-owned battery and processing writes use the same PowerNet/network setter but suppress self-invalidating scheduler churn. Energy dirtiness is no longer broadened to recipe dirtiness, because power changes do not invalidate the cached vanilla furnace recipe.

The old visual boundary behavior is retained: completing one item leaves the lit block in place when the next item is immediately eligible and sufficiently powered, while a stopped or invalid operation switches off. Progress and power remain synchronized at their gameplay-visible accounting cadence, with the existing once-per-second baseline packet. There is no Electric Furnace redstone/comparator control path to migrate, and its sided inventory remains slots 0-2 with the same insertion/extraction rules.

### Persistent operation state and reconstruction

The existing `power` and `progress` keys remain unchanged. Added TileEntity NBT stores the active-operation flag, cooldown, next authoritative accounting tick, effective duration, and effective per-tick consumption. Old saves default those additions conservatively and infer activity from existing progress. Recipe objects, item references, runtime keys, queue records, and scheduler internals are not persisted; the inventory remains the recipe identity and is revalidated against `FurnaceRecipes`.

On lifecycle bind, the furnace refreshes upgrade-derived timing and recipe/output eligibility before it trusts persisted work. A valid future due tick is recreated as a typed runtime transition. An overdue operation performs at most one current accounting step rather than simulating unloaded time. A newly deserialized TileEntity cannot execute a dormant runtime transition before its lifecycle reevaluation initializes the cached state. The per-world scheduler remains transient, so server restart and world reload always reconstruct from TileEntity NBT.

Chunk unload unbinds the TileEntity without simulating it. The logical entry may retain dirty causes and transition ownership while the world remains loaded; rebind requeues dormant work. Actual destruction removes the generation-bearing entry and cancels its work, so a later furnace at the same coordinates cannot receive the stale callback.

### Invalidation coverage and compatibility polling

Standard `IInventory` setters, extraction, GUI slots, shift-click handling, and hopper automation pass through `TileEntityMachineBase` and issue inventory + recipe dirtiness. The furnace cancels its current accounting transition on those mutations and invalidates its upgrade cache for slot 3. External PowerNet delivery uses `setPower()` and issues energy dirtiness. Lifecycle bind supplies the reconstruction cause, and common configuration mutations use the existing configuration cause. Completion mutates the input/output arrays internally, then immediately refreshes cached eligibility before scheduling the next step.

The remaining gap is code that mutates the public `slots[]` array or an existing `ItemStack` in place without calling an inventory method. No such external Electric Furnace mutation was found in the inspected in-tree call sites, but the wider 1.7.10 mod and third-party integrations cannot be assumed to obey the setter contract. The deterministic 20-tick bucket therefore compares upgrade content and recipe/output eligibility, and wakes dormant battery work. This bounds compatibility recovery without restoring the full legacy recipe/upgrade path every tick. A direct mutation that changes one valid recipe into another while output remains compatible does not require cancellation; the legacy furnace likewise retained progress in that case and consumed the recipe present at completion.

### Retained on/off block identity

Forge 1.7.10 block replacement removes the old TileEntity and may validate a temporary TileEntity for the target block before `updateBlockState()` restores the original instance. The Phase 0 invalidate bracket alone allowed that temporary instance to bind the vacant position and allocate or steal the logical generation. The runtime now records the retained position and exact owner during the bracket, refuses a different temporary binding there, suppresses a duplicate lifecycle dirty signal when the original instance rebinds, and clears the guard after the swap. Normal lit/unlit swaps consequently retain the same generation and scheduled ownership; real destruction still removes the entry and a new placement allocates a new generation.

### Lessons and later phases

The first migration shows that continuous energy semantics may still require a lightweight scheduled accounting transition even when recipe and upgrade evaluation is event-driven. Later migrations must separate cheap resource accounting from expensive eligibility calculation instead of assuming every process can jump directly to a completion tick. They must also audit direct slot/tank mutation, GUI synchronization, completion consumption, and block/proxy replacement for each family rather than copying this furnace strategy wholesale.

The small assembler was selected for the Phase 2 migration below. Chemplant still needs direct tank and slot mutation coverage. Multiblocks, boilers, reactors, and continuous environmental machines remain on their legacy paths.

Source inspection covered the Electric Furnace block, TileEntity, base inventory/lifecycle and packet paths, vanilla recipe lookup, upgrade manager, battery charging, PowerNet setter, sided automation, container/GUI, NBT, pollution, block replacement, runtime ordering, and in-tree references. Static diff checks completed without whitespace errors, and the targeted offline `compileJava` task completed successfully. Minecraft was not launched; in-game checks remain necessary for save/reload, chunk unload/rebind, rapid lit/unlit swaps and replacement, power starvation/recovery, battery variants, automation, upgrades during progress, soot cadence, and GUI/visual parity.

## 2026-09-25 20:51 — Phase 2 regular Assembler migration

### Actual machine and lane model

`TileEntityMachineAssembler` has exactly one recipe lane (`getRecipeCount() == 1`): template slot 4, input slots 6–17, output slot 5, upgrade slots 1–3, and battery slot 0. Its single operation uses typed accounting task slot 0. The eight lanes of `TileEntityMachineAssemfac` come from the shared `TileEntityMachineAssemblerBase` but the assembly factory remains `LEGACY`; this phase does not establish simultaneous multi-lane execution or shared-resource completion ordering. The assembler has no active/inactive block swap, redstone gate, or pollution step. Client sound and rendering use the existing `isProgressing` and progress packet fields.

The old base tick charged its battery, exported at most one output/template item, pulled templates and ingredients from a neighboring inventory, then checked template, power, all ingredients, and output capacity before charging HE and advancing progress. Inputs were consumed and output created only on completion. `TileEntityMachineAssembler.updateEntity()` then converted old multiblock metadata if needed, refreshed PowerNet connections, recalculated speed/consumption from upgrades, and sent a GUI packet. Upgrade effects consequently applied to the next processing tick. Losing power or blocked input/output reset progress to zero rather than pausing it.

The migrated controller uses `EVENT_DRIVEN | SCHEDULED | COARSE_20`. Standard inventory or external energy changes cancel its slot-0 task and coalesce into one machine-level dirty evaluation. That evaluation uses the existing template recipe cache, checks required ingredients and output space, and classifies the lane as eligible or idle. Active work, useful battery charging, and the one boundary tick after a completion are accounted with a typed one-tick transition. This interval preserves per-tick battery discharge, per-tick HE consumption, progress, starvation reset, completion timing, and GUI/sound cadence. It does not reserve energy or perform an expensive ingredient/output scan on every active tick. The authoritative completion boundary revalidates the current recipe, ingredients, and output capacity before consuming or producing; completion refreshes eligibility immediately for the next item. The original upgrade formulas and next-tick application order are retained.

### Adjacent inventory and recipe invalidation

The neighboring inventories are a real dependency, not just a speculative compatibility concern. The assembler pulls templates from `TileEntityCrateTemplate`, pulls matching ingredients from its input position, and pushes output or a completed template to its output position. No general event reports arbitrary neighbor inventory content changes to this controller. Immediate transfer checks remain in `updateEntity()` when a template/input is needed or there is output/template work to export; successful direct input/template mutations issue one inventory/recipe dirty signal. Existing output removal passes through the base inventory hook. Touched neighboring inventories are marked dirty on actual transfers so their changed contents are saved. When sufficient internal ingredients are already present, the costly neighbor ingredient loop is skipped; when no output/template can be exported, the output neighbor scan is skipped. This preserves prompt discovery of newly available adjacent items where transfer matters without a global observer.

GUI slots, ordinary inventory setters/extraction, and inventory-capable multiblock proxies delegate to `TileEntityMachineBase` hooks. Container merges and other integrations may mutate an existing `ItemStack` in place without a setter, and other code can write public `slots[]` directly. The deterministic 20-tick bucket therefore rechecks template identity/content, ingredient and output eligibility, upgrades, battery wakeup, and unreported direct power writes. A missed direct mutation can affect progress until this bound, but the completion recheck prevents production from stale inputs or a full output. Neither arbitrary third-party inventory mutation nor a direct write can be treated as an immediate event. Neighbor inventory contents are discovered through the retained transfer loop, and adjacency/topology does not require broad dirty fanout.

The existing assembler recipe cache now includes a generation advanced by `AssemblerRecipes` registration/deletion and the in-tree ingredient alteration. `/ntmreload` therefore invalidates cached template results even if the same template object stays in the slot. The regular assembler checks that scalar generation before transfer/accounting and revalidates work when it changes. Recipe identity remains the template item/meta/NBT and the current registry entry; no recipe object is persisted. Direct out-of-band mutation of recipe objects without the registration API remains outside this revision seam.

### Persistence, lifecycle, and later work

The existing `power`, `progress[]`, and `maxProgress[]` NBT keys remain intact. New regular-assembler NBT records the next accounting boundary, active presentation flag, and pending template-switch flag; effective speed and consumption remain derived from upgrades in their existing one-tick order. On load or rebind, the controller rechecks template/input/output state before reconstructing a valid next-tick transition. Stale or overdue due ticks perform at most one current accounting step. A dormant task cannot run against an uninitialized rebound TileEntity, and unload never performs offline production. A real break removes generation-owned tasks. The old-metadata multiblock conversion explicitly removes the outgoing controller entry and restores the newly allocated generation after copying its saved state into the replacement TileEntity.

No `MachineRuntime` scheduler change or new diagnostics were needed: its existing full generation key and `(taskType, taskSlot)` ownership cover this one lane. The source-level reduction is specific: stable active work no longer calls `canProcess()` and its allocation-heavy full ingredient/output checks every tick; stable sufficient input also skips adjacent ingredient loading; upgrade inspection and expensive eligibility are dirty-driven with a distributed 20-tick audit. Metadata-aware reuse removes fresh `DirPos` arrays from steady connection and transfer calls. PowerNet connection attempts, needed neighbor transfers, and lightweight active accounting still recur each tick because they are part of observed gameplay timing. These are structural claims, not measured CPU or allocation results.

Safe future offline production would need a deterministic record of recipe version, input/output and adjacent inventory ownership, upgrade and energy availability over elapsed time, transfer order, and commit/revalidation rules. None is available from a due tick alone, so this phase keeps unloaded controllers dormant. The regular Chemplant was not yet ready for this cutover: its direct slot/tank writes and external fluid network changes needed separate inspection. The multi-recipe ChemplantBase variants were not part of this assertion.

Source inspection covered the regular assembler, assembly factory boundary, shared base, block/proxies, neighboring crate inventory, recipe reload, inventory helpers, GUI/container/audio, PowerNet setter, NBT, and runtime ordering. The targeted offline `compileJava` task completed successfully. Minecraft was not launched. Dedicated-server validation remains for rapid recipe/template changes, reload, input/output automation, output-full transitions, power loss, upgrades during progress, save/load, unload/rebind, old-metadata conversion, and GUI/sound parity.

## 2026-09-25 23:02 — Phase 3 regular Chemplant migration

The regular `TileEntityMachineChemplant` extends `TileEntityMachineBase` directly, not `TileEntityMachineChemplantBase`. It has one scalar `progress`, one template slot (4), one independently selected recipe, and one processing lane mapped to `(TASK_ACCOUNTING, slot 0)`. Four 24,000 mB tanks belong to that one operation: input tanks 0–1 and output tanks 2–3. Item inputs are slots 13–16, item outputs 5–8, fluid containers 9–12 and 17–20, upgrades 1–3, and battery 0. There is no simultaneous lane execution or same-machine completion ordering to prove here. The factory/large Chemplant code paths remain legacy. Their per-lane tank/slot ownership and any shared-resource completion order must be established separately; this phase makes no heap-ordering claim.

The legacy server tick charged the battery; processed fluid containers with 40-tick dual-load delays; transferred neighboring items; refreshed PowerNet/fluid subscriptions and sent output fluid; recomputed upgrades; then set recipe-dependent tank type/pressure and checked power, all fluid/item inputs, all output space, and oxygen. If eligible it consumed oxygen from an atmosphere blob where relevant, spent power, advanced progress, recomputed duration, and consumed/produced items and fluids at completion. Eligibility or power loss reset progress; upgrades applied before that tick's processing. The migration retains the container-delay counters, per-tick output fluid sends, neighbor item export, needed input pulls, 20-tick subscription check, client sound/particles, and one-tick accounting for battery, power, progress and oxygen. Upgrade-derived speed and consumption use the original formula before accounting; no up-front power reservation or HE resource was found. A stable idle controller no longer runs the full `canProcess` recipe/fluid/item/output scan every tick; stable active accounting checks compact tank state and defers full ingredient/output checks to dirty reevaluation and the authoritative completion boundary. Output removal and demanded input loading still run at their legacy cadence.

Standard inventory setters/extraction mark inventory/recipe dirty; direct neighboring input pulls explicitly signal it and mark the neighbor dirty. External PowerNet `setPower` changes signal energy dirty. Chemplant-owned container loads/unloads and standard fluid transceiver receive/send paths signal fluid dirty and cancel stale accounting. The four-tank snapshot compares fluid identity, pressure, fill and capacity without cloning stacks; active accounting detects tank changes before work, while the distributed 20-tick audit also fingerprints tank state and all 21 inventory slots to catch direct `slots[]`, in-place `stackSize`/NBT, and `FluidTank.setFill` writes without callbacks. A direct item mutation can remain unobserved until that audit, but completion revalidates the current recipe and authoritative item/fluid/output state before consuming anything. Other callers holding `getAllTanks()` or direct tank references can still bypass immediate callbacks; this bounded audit is the fallback, not a universal fluid-system rewrite. The regular Chemplant uses the HBM fluid transceiver rather than a Forge `IFluidHandler` implementation. Neighboring item inventory changes are found through the retained input transfer when needed, and output changes through the retained export path or 20-tick local fingerprint; there is no generic adjacent-inventory event.

Chemplant recipe lookup remains the existing ID-index map. A monotonic recipe generation now advances when recipes register or are deleted, so `/ntmreload` invalidates the TileEntity's cached recipe even when the template object and metadata stay fixed. The recipe object is never saved. Existing `power`, `progress`, and tank NBT keys remain; new NBT records the next accounting boundary and client active flag. On bind/rebind the controller resolves the current template, rechecks tanks/items/outputs, recomputes upgrades, and only reconstructs valid work. An overdue transition performs at most one current accounting step; unloaded chunks have no elapsed-time catch-up. No `MachineRuntime` queue or tank-layer API change was needed for a one-lane machine. Shared-resource double-consumption is prevented at the only completion boundary by synchronous revalidation; this is not evidence for multi-lane races. Concurrent operation slots, deterministic same-tick lane ordering, and cross-lane resource invalidation remain the Assembly Factory's next distinct problem, along with its template switches and per-lane adjacent transfers.

These are structural source observations, not measured performance or in-game validation. Dedicated-server tests remain for fluid networks, container delays, direct tank/slot mutations, atmosphere consumption, recipe reload, fast template swaps, upgrade and energy changes during progress, save/load and chunk rebind, neighbor automation, GUI/audio parity, and tank-type changes. Safe persistent unloaded graph simulation would additionally require time-bounded fluid-network and atmosphere state, input/output ownership, recipe versions, and transactional resource commits; a saved due tick alone is insufficient.
