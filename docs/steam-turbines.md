...
Trip valves remain closed until reset. Use the turbine's built-in lever to reset a trip, or apply a redstone signal to the turbine controller for a remote reset. Ensure the blocked steam or electrical path is corrected before resetting it, or the turbine will trip again. The turbine's look overlay reports synchronized inlet and exhaust amounts, including on the client.
