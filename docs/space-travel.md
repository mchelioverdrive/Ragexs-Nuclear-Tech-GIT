...
Orbit sky rendering intentionally follows the HBM Space reference under `DONOTMODIFY/hbmspace`. Planet visuals in orbit are square billboard-style celestial renders driven by branch-local satellite orbital metrics, not spherical geometry around the station.
