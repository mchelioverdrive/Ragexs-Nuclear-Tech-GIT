...
| `DARK_ADAPTATION_STRENGTH` | `1.0` | Scales the bounded shadow effect (runtime-clamped to 0–2); black-recovery gain saturates at 1 so large values cannot destabilize color math. |
| `DARK_ADAPTATION_SCOTOPIC_FLOOR` | `0.055` | Legacy-compatible control for the bounded perceptual black-geometry display target. The default maps to a 0.140 fully illuminated/adapted target before environmental, shape, and central-vision factors (runtime-clamped to 0–0.15). |
...
| `DARK_ADAPTATION_HD_SKY_CARRIER` | `0.06` | Sky-factor information carrier (runtime-clamped to 0–0.12). It affects sky light only; it is not a final RGB floor or an eye-adaptation brightness control. |
...
With Hardcore Darkness installed, the compatibility hook changes only the lightmap's original `0.95`/`0.05` sky-factor pair. The resulting factor is `sunBrightness * (1 - carrier) + carrier`, preserving real outdoor texture information before Hardcore Darkness can erase it. It does not restore the later global RGB floor: zero-skylight sealed caves therefore remain black, while RTM's post-process still controls perceived darkness, desaturation, contrast, blur, noise, and adaptation.

When depth capture is available, environmental illumination is converted with a bounded `ambient^0.30` perceptual curve, then combined with 25% cone and 75% rod recovery and a target display luminance. This supplies a dim grayscale floor on valid mathematical-black world geometry while linearized neighboring depth adds at most 20% shape emphasis. A broad flat wall therefore remains faintly visible outdoors rather than requiring an edge to create light. Zero ambient input still maps to exactly zero recovery. Cleared-depth sky and void receive no synthetic floor. Mathematical-black pixels no longer contain texture or color information—Hardcore Darkness has already discarded it—so RTM can reconstruct only low-acuity achromatic geometry from depth, not restore original texture detail or color.
