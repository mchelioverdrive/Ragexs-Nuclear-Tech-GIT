...
| `DARK_ADAPTATION_SCOTOPIC_FLOOR` | `0.055` | Maximum grayscale recovery for mathematically black geometry under full outdoor scotopic illumination (runtime-clamped to 0–0.15). |
...
| `DARK_ADAPTATION_DEBUG` | `false` | Shows rendered-scene exposure, cone/rod state, environmental scotopic/sky/moon/weather values, the GPU metering path, shader/framebuffer/depth status, Angelica detection, and the latest failure. |
...
When depth capture is available, environmental illumination supplies a dim grayscale floor on valid mathematical-black world geometry while depth gradients add silhouettes and local shape. A broad flat wall therefore remains faintly visible outdoors rather than requiring an edge to create light. Cleared-depth sky and void receive no synthetic floor. Mathematical-black pixels no longer contain texture or color information—Hardcore Darkness has already discarded it—so RTM can reconstruct only low-acuity achromatic geometry from depth, not restore original texture detail or color. Ordinary nonzero RGB shadow recovery remains available if depth capture is unsupported, and the color-copy pass can operate on the normal backbuffer even when Minecraft's framebuffer setting is disabled.
