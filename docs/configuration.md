...
| `DARK_ADAPTATION_DEBUG` | `false` | Shows every recovery multiplier, requested and used strength, expected broad/shape luminance, GPU/depth/shader status, and the latest failure. |
| `DARK_ADAPTATION_DEBUG_VIEW` | `0` | Debug-only output: `0` normal, `1` geometry mask (white valid world depth, black cleared sky/void), `2` mathematical-black recovery contribution. Ignored unless debug is enabled. |
...
The compact debug overlay reports `sceneExposure`, cone/rod/effective adaptation, requested/used strength, ambient/perceived ambient, eye recovery, sky/night/moon/weather factors, expected broad/shape black luminance, depth availability and geometry-mask verification guidance, shader state, and failure reason. Use debug view `1` to establish that the copied depth texture contains white nearby terrain; use view `2` to isolate the calculated recovery signal.
