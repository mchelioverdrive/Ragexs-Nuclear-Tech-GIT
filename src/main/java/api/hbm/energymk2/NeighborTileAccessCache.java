 ...
 */
public class NeighborTileAccessCache {
	...
	static {
		MinecraftForge.EVENT_BUS.register(new NeighborTileAccessCache());
	}
	...
	private static class TileAccessEntry {
		...
		private boolean isValid(World world, int x, int y, int z, long now) {
			...
			return !tile.isInvalid() && tile.getWorldObj() == world && tile.xCoord == x && tile.yCoord == y && tile.zCoord == z;
		}
	}
}
