...
public class BlockGraphiteFuel extends BlockGraphiteDrilledTE implements IToolable, IBlowable {
	...
	public TileEntity createNewTileEntity(World world, int meta) {
		...
		if((meta & 8) != 0)
			...
			pile.progress = pile.maxProgress / 4;
	...
	public boolean onScrew(World world, EntityPlayer player, int x, int y, int z, int side, float fX, float fY, float fZ, ToolType tool) {
		...
		if(!world.isRemote) {
			...
			if(tool == ToolType.HAND_DRILL) {
				...
				player.addChatComponentMessage(new ChatComponentText("BURNUP: " + pile.progress + "/" + pile.maxProgress).setChatStyle(new ChatStyle().setColor(EnumChatFormatting.YELLOW)));
				player.addChatComponentMessage(new ChatComponentText("NEUTRON FLUX: " + pile.lastNeutrons).setChatStyle(new ChatStyle().setColor(EnumChatFormatting.YELLOW)));
				if((meta & 8) == 8)
	...
	public void applyFan(World world, int x, int y, int z, ForgeDirection dir, int dist) {
		TileEntityPileFuel pile = (TileEntityPileFuel) world.getTileEntity(x, y, z);
		pile.heat -= Math.max(0.05, pile.heat * 0.0035);
	}
}
