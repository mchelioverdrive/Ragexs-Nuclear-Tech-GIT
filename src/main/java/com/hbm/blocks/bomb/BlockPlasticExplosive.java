...
public class BlockPlasticExplosive extends BlockDetonatable implements IBomb {
	...
	public BombReturnCode explode(World world, int x, int y, int z) {
		...
		if(!world.isRemote) {
			//todone differentiate between c4 and semtex
			//assume both are 250kg
			//if (this.block_c4)
			//1 meter cubed of both substances
			//RDX
			if(this == ModBlocks.block_c4) {
				new ExplosionVNT(world, x + 0.5, y + 0.5, z + 0.5, 15).makeStandard().setBlockProcessor(new BlockProcessorStandard().setNoDrop()).explode();
			}
			//else if (this.block_semtex)
			//PETN
			else if(this == ModBlocks.block_semtex) {
				new ExplosionVNT(world, x + 0.5, y + 0.5, z + 0.5, 16).makeStandard().setBlockProcessor(new BlockProcessorStandard().setNoDrop()).explode();
			}
		}
