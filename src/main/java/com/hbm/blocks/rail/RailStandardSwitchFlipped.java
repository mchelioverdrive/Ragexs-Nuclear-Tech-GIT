...
public class RailStandardSwitchFlipped extends BlockRailWaypointSystem implements IRenderBlock {
	...
	public void renderInventory(Tessellator tessellator, Block block, int metadata) {
		...
		ObjUtil.renderPartWithIcon(ResourceManager.rail_standard_switch_flipped, "Rail", this.blockIcon, tessellator, 0, false);
		tessellator.draw();
	}
	...
	public void renderWorld(Tessellator tessellator, Block block, int meta, IBlockAccess world, int x, int y, int z) {
		...
		ObjUtil.renderPartWithIcon(ResourceManager.rail_standard_switch_flipped, "Rail", this.blockIcon, tessellator, rotation, true);
		
		TileEntity tile = world.getTileEntity(x, y, z);
		
		if(tile instanceof TileEntityRailSwitch) {
			TileEntityRailSwitch sw = (TileEntityRailSwitch) tile;
			ObjUtil.renderPartWithIcon(ResourceManager.rail_standard_switch_flipped, sw.isSwitched ? "SignTurn" : "SignStraight", this.iconSign, tessellator, rotation, true);
		}
