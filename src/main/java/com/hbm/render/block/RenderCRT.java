...
public class RenderCRT implements ISimpleBlockRenderingHandler {
	...
	public void renderInventoryBlock(Block block, int metadata, int modelId, RenderBlocks renderer) {
		...
		ObjUtil.renderWithIcon(ResourceManager.crt, iicon, tessellator, (float) Math.PI * -0.5F, false);
		tessellator.draw();
	...
	public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, Block block, int modelId, RenderBlocks renderer) {
		...
		ObjUtil.renderPartWithIcon(ResourceManager.crt, "Monitor", iicon, tessellator, rotation, true);
		if(metaOrig >= 8) tessellator.setBrightness(240);
		ObjUtil.renderPartWithIcon(ResourceManager.crt, "Screen", iicon, tessellator, rotation, true);
		tessellator.setBrightness(brightness);
		tessellator.addTranslation(-x - 0.5F, -y, -z - 0.5F);
