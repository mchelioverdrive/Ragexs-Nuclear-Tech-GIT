		...
		if(!worldObj.isRemote) {
			...
			if(started) {
				...
				if(timer <= 0) {
					...
					// once exploded, nothing else to do
				}
			}

			// --- Defuse progression on server ---
			if(defusing) {
				// Validate the player exists and is within distance and sneaking
				...
				if(cancel) {
					...
					defuserName = "";
					markDirty();
					worldObj.playSoundEffect(xCoord, yCoord, zCoord, "hbm:item.defuseStop", 1.0F, 1.0F);
				} else {
					...
					if(defuseTicks % 20 == 0) {
						worldObj.playSoundEffect(xCoord, yCoord, zCoord, "hbm:item.defuseTick", 0.8F, 1.0F);
					}
					...
					if(defuseTicks >= DEFUSE_TIME_TICKS) {
						...
						worldObj.playSoundEffect(xCoord, yCoord, zCoord, "hbm:item.defuseComplete", 1.0F, 1.0F);
						...
						if(p != null) {
							p.addChatMessage(new net.minecraft.util.ChatComponentText("\u00a7aBomb disarmed."));
						}
					} else {
						markDirty();
					}
