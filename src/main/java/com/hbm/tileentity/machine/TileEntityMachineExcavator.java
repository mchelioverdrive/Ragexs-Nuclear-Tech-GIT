...
public class TileEntityMachineExcavator extends TileEntityMachineBase implements IEnergyReceiverMK2, IFluidStandardReceiver, IControlReceiver, IGUIProvider, IUpgradeInfoProvider, IFluidCopiable {
	...
	protected void breakSingleBlock(Block b, int x ,int y, int z) {
		...
		// XRadar Compatibility: Fetch stone drops from config
		items.addAll(getXradarStoneDrops(b, worldObj.getBlockMetadata(x, y, z)));
	...
	private List<ItemStack> getXradarStoneDrops(Block block, int metadata) {
		List<ItemStack> extraDrops = new ArrayList<>();
		File configFile = new File("config/stonedrops.json");

		if (!configFile.exists()) return extraDrops;

		try (FileReader reader = new FileReader(configFile)) {
			Gson gson = new Gson();
			List<XradarDrop> dropList = gson.fromJson(reader, new TypeToken<List<XradarDrop>>(){}.getType());
			Random rand = new Random();

			for (XradarDrop drop : dropList) {
				if (rand.nextFloat() < drop.rarity) { // Uses 'rarity' instead of 'chance'
					...
					if (parts.length == 2) {
						...
						if (item != null) {
							extraDrops.add(new ItemStack(item, drop.count, drop.meta));
						}
					}
				}
			}
	...
	class XradarDrop {
		String item;
		int count;
		float rarity; // Ensure this matches the JSON field name
		int meta;

		// Default constructor for Gson
		public XradarDrop() {}

		// Method to check if a drop applies to a block
		public boolean matches(Block block, int metadata) {
			return true; // Extend this logic if needed
		}
	}
