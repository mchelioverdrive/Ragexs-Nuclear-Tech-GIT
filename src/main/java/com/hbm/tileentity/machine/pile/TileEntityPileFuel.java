...
public class TileEntityPileFuel extends TileEntityPileBase implements IPileNeutronReceiver {
	...
	private int react() {
		...
		int reaction = (int) (this.neutrons *
			(1D - ((double)this.heat / (double)this.maxHeat) * 0.5D));

		// thermal expansion / moderator geometry effects
		reaction = Math.max(reaction, 0);
		...
		// === Passive decay heat ===
		// irradiated uranium keeps producing residual heat
		// scales with burnup, tiny for realism
		if(this.progress > 0) {

			double burnup =
				(double)this.progress / (double)this.maxProgress;

			this.heat += Math.max(1,
								  (int)(burnup * 2D));
		}

		// no fission happening
		if(reaction <= 0)
			return lastProgress;

		// active fission heating
		//this.heat += reaction;
		this.heat += Math.max(1, reaction / 6);
