...
public abstract class TileEntityPileBase extends TileEntity {
	...
	protected void castRay(int flux, int range) {
		...
		for(float i = 1; i <= range; i += 0.5F) {
			...
			if(b == ModBlocks.concrete || b == ModBlocks.concrete_smooth || b == ModBlocks.concrete_colored || b == ModBlocks.brick_concrete)
				flux *= 0.65D;
			if(b == ModBlocks.concrete_asbestos)
				flux *= 0.55D;
			...
			if(te instanceof IPileNeutronReceiver) {
				...
				if(b == ModBlocks.block_graphite
					|| b == ModBlocks.block_graphite_drilled) {
			...
			if(entities != null)
				for(EntityLivingBase e : entities) {
					...
					ContaminationUtil.contaminate(e, HazardType.RADIATION, ContaminationType.CREATIVE, flux / 20F);
				}
		}
	}
}
