...
public class ArmorHEV extends ArmorFSBPowered {
	...
	private void renderOverlay(RenderGameOverlayEvent.Pre event, EntityPlayer player) {
		...
		for(int i = 0; i < 4; i++) {
			...
			c += (double) item.getStoredEnergyQuanta(armor) / (double) item.getEnergyCapacityQuanta(armor);
		}
		...
		if(radiation > 0) {
			...
			Minecraft.getMinecraft().fontRenderer.drawString(delta + " mSv/s", dX, dY, 0xFF0000);
		}
